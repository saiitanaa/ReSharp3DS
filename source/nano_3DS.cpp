...
extern "C" {
    ...
    void HAL_Assert(const char* Func, int Line, const char* File) {
        ...
        while (aptMainLoop()) {
            ...
            // AJOUTE CES DEUX LIGNES POUR FORCER L'AFFICHAGE !
            gfxFlushBuffers();
            gspWaitForVBlank();
        }
        exit(0);
...
extern "C" {
    void HeapLocation(unsigned char*& baseAddress, unsigned int& sizeInBytes) {
        printf("[DEBUG] HeapLocation (C++) appele par le CLR !\n");
    ...
    HRESULT nanoCLR_Initialize(const void* settings) {
        ...
        // C'est cette ligne qui va appeler ton nouveau HeapLocation C++
        CLR_RT_Memory::Reset();
...
HRESULT CLR_HW_Hardware::CreateInstance() {
    printf("[STUB] CLR_HW_Hardware::CreateInstance appele\n");
    return S_OK;
}
